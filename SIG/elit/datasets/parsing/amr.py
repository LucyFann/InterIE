...
from elit.components.sig.sig_parser.sigio import SIGIO
from elit.components.sig.sig_parser.data import END, DUM, list_to_tensor, lists_of_string_to_tensor, NIL, REL
from elit.components.sig.sig_parser.transformer import SelfAttentionMask
from elit.transform.transformer_tokenizer import TransformerSequenceTokenizer
from elit.common.util import merge_list_of_dict


class AbstractMeaningRepresentationDataset(TransformableDataset):
    def load_file(self, filepath: str):
        for tok, lem, pos, ner, sig in SIGIO.read(filepath):
            yield {'token': tok, 'lemma': lem, 'pos': pos, 'ner': ner, 'sig': sig}


def generate_oracle(sample: dict):
    sig = sample.get('sig', None)
    if sig:
        concept, edge, _ = sig.root_centered_sort()
...
def batchify(data, vocabs: VocabDict, unk_rate=0., device=None, squeeze=False,
    ...
    if 'sig' in data:
        ...
        for sig in data['sig']:
            if levi_graph == 'kahn':
                concept_i, edge_i = sig.to_levi(rel_vocab.get_frequency, shuffle=shuffle_sibling)
            else:
                concept_i, edge_i, _ = sig.root_centered_sort(rel_vocab.get_frequency, shuffle=shuffle_sibling)
...
def levi_sig(concept, edge, extra_arc=False):
    concept_with_rel = []
    edge_with_rel = []
