...
from elit.utils.io_util import get_resource, run_cmd, pushd
from elit.utils.log_util import flash

_SMATCH_SCRIPT = 'https://github.com/ChunchuanLv/sig-evaluation-tool-enhanced/archive/master.zip#evaluation.sh'
_FAST_SMATCH_SCRIPT = 'https://github.com/jcyk/SIG-gs/archive/master.zip#tools/fast_smatch/compute_smatch.sh'
...
def post_process(pred, sig_version):
    pred = os.path.realpath(pred)
    utils_tar_gz = get_sig_utils(sig_version)
    util_dir = get_resource(utils_tar_gz)
    stog_home = get_resource('https://github.com/jcyk/SIG-gs/archive/master.zip')
    with pushd(stog_home):
        run_cmd(
            f'python3 -u -m stog.data.dataset_readers.sig_parsing.postprocess.postprocess '
            f'--sig_path {pred} --util_dir {util_dir} --v 2')
    return pred + '.post'


def get_sig_utils(sig_version):
    if sig_version == '1.0':
        utils_tar_gz = 'https://www.cs.jhu.edu/~s.zhang/data/SIG/sig_1.0_utils.tar.gz'
    elif sig_version == '2.0':
        utils_tar_gz = 'https://www.cs.jhu.edu/~s.zhang/data/SIG/sig_2.0_utils.tar.gz'
    elif sig_version == '3.0':
        utils_tar_gz = 'https://od.hankcs.com/research/sig2020/sig_3.0_utils.tgz'
    else:
        raise ValueError(f'Unsupported SIG version {sig_version}')
    return utils_tar_gz
