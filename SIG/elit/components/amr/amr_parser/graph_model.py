...
class GraphAbstractMeaningRepresentationModel(nn.Module):
    def __init__(self,
                 vocabs: VocabDict,
                 concept_char_dim,
                 concept_dim,
                 ...
                 encoder=None,
                 **kwargs):
        super(GraphAbstractMeaningRepresentationModel, self).__init__()
        self.vocabs = vocabs
    ...
    def encode_step_with_bert(self, lem, batch):
    ...
    def predict(self, batch, beam_size, max_time_step, min_time_step=1, **kwargs):
        with torch.no_grad():
            word_repr, word_mask, probe = self.encode_step_with_bert(batch['lem'], batch=batch)
