...
import torch
from stog.data.dataset_readers.sig_parsing.sig import SIGGraph
from stog.data.dataset_readers.sig_parsing.node_utils import NodeUtilities
...
from elit.components.sig.sig_parser.data import END, DUM, NIL
from elit.components.sig.sig_parser.graph_model import GraphAbstractMeaningRepresentationModel
from elit.components.sig.sig_parser.postprocess import PostProcessor
from elit.components.sig.sig_parser.work import parse_data, parse_data_
...
from elit.metrics.sig.smatch_eval import smatch_eval, post_process, SmatchScores, get_sig_utils
from elit.metrics.f1 import F1_
from elit.utils.io_util import tempdir, get_resource
...
class GraphAbstractMeaningRepresentationParser(TorchComponent):
    ...
    def evaluate_dataloader(self,
        ...
        parse_data(model, pp, data, input, output, max_time_step=80 if model.squeeze else 100, h=h)
    ...
    def build_model(self, training=True, **kwargs) -> torch.nn.Module:
        ...
        model = GraphAbstractMeaningRepresentationModel(self.vocabs,
    ...
    def build_dataloader(self, data, batch_size, shuffle=False, device=None, logger: logging.Logger = None,
                         ...
                         **kwargs) -> DataLoader:
        dataset, lens = self.build_dataset(data, logger, training=shuffle, transform=self.config.encoder.transform())
        if batch_max_tokens:
    ...
    def build_batchify(self, device, shuffle, shuffle_sibling=None):
        ...
        tokenizer = self.config.encoder.transform() if self.config.get('encoder', None) else None
    ...
    def build_dataset(self, data, logger: logging.Logger = None, training=True, transform=None):
        dataset = AbstractMeaningRepresentationDataset(data, generate_idx=not training)
        if self.vocabs.mutable:
            self.build_vocabs(dataset, logger)
            self.vocabs.lock()
            self.vocabs.summary(logger)
        lens = [len(x['token']) + len(x['sig'] if 'sig' in x else []) for x in dataset]
    ...
    def predict(self, data: Union[str, List[str]], batch_size: int = None, **kwargs):
        ...
        dataloader = self.build_dataloader(samples, device=self.device,
                                           **merge_dict(self.config, overwrite=True, batch_size=batch_size))
        pp = PostProcessor(self.vocabs['rel'])
        results = list(parse_data_(self.model, pp, dataloader))
        for i, each in enumerate(results):
            sig_graph = SIGGraph(each)
            self.sense_restore.restore_graph(sig_graph)
            results[i] = sig_graph
        if flat:
            return results[0]
        return results
    ...
    def on_config_ready(self, **kwargs):
        ...
        utils_dir = get_resource(get_sig_utils(self.config.sig_version))
