...
import numpy as np
from sig_parser.extract import read_file, write_vocab
from elit.components.sig.sig_parser.data import REL
from elit.datasets.parsing.sig import linearize, levi_sig
import tempfile
...
def batchify(data, vocabs,unk_rate=0., shuffle_siblings=True, levi_graph=False, extra_arc=False):
    ...
    #_entity_type = ListsToTensor([[CLS] + x['entity_type'] for x in data], vocabs['entity_type'], unk_rate=unk_rate)
    # _mention_type = ListsToTensor([[CLS] + x['mention_type'] for x in data], vocabs['mention_type'], unk_rate=unk_rate)
    # _entity_subtype = ListsToTensor([[CLS] + x['entity_subtype'] for x in data], vocabs['entity_subtype'], unk_rate=unk_rate)
    #_trigger_type = ListsToTensor([[CLS] + x['trigger_type'] for x in data], vocabs['trigger_type'], unk_rate=unk_rate)
    ...
    for x in data:
        sig = x['sig']
        if levi_graph == 'kahn':
            # because concept wand rel go to concept vocab
            concept_i, edge_i = sig.to_levi(vocabs['concept'].priority, shuffle=shuffle_siblings)
        else:
            concept_i, edge_i, _ = sig.root_centered_sort(vocabs['rel'].priority, shuffle=shuffle_siblings)
        concept.append(concept_i)
        edge.append(edge_i)
    # tokidx rel_label
    if levi_graph is True:
        concept_with_rel, edge_with_rel = levi_sig(concept, edge, extra_arc=extra_arc)
    ...
    #_data_ace = numberize(data, bert_tokenizer, vocabs_ace)
    #_data_ace = collate_fn(_data_ace)
...
class DataLoader(object):
    def __init__(self, vocabs, lex_map, filename, batch_size, for_train, shuffle_siblings=True, levi_graph=False,
        ...
        for sig, token, lemma, pos, ner in zip(*read_file(filename)):
            if for_train:
                _, _, not_ok = sig.root_centered_sort()
            ...
            datum = {'sig': sig, 'tok': token, 'lem': lemma, 'pos': pos, 'ner': ner, \
        ...
        print("Get %d SIGs from %s" % (len(self.data), filename))
        self.vocabs = vocabs
    ...
    def load_data(self):
        ...
        # print("lens", len(self.sig_graphs))
    ...
    def __iter__(self):
        ...
        if self.train:
            random.shuffle(idx)
            idx.sort(key=lambda x: len(self.data[x]['tok']) + len(self.data[x]['sig']))

        batches = []
        num_tokens, data = 0, []
        for i in idx:
            num_tokens += len(self.data[i]['tok']) + len(self.data[i]['sig'])
            data.append(self.data[i])
            if num_tokens >= self.batch_size:
                sz = len(data) * (2 + max(len(x['tok']) for x in data) + max(len(x['sig']) for x in data))
                if sz > GPU_SIZE:
