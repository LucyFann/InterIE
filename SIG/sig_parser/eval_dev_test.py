...
from elit.metrics.sig.smatch_eval import smatch_eval, SmatchScores
...
from sig_parser.work import predict


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('load_path', type=str, help='The folder containing checkpoints')
    parser.add_argument('--version', type=str, help='SIG data version', default='2.0')
    ...
    for dev in devs:
        ...
        if not os.path.isfile(f'{load_path}/{pred_path}'):
            print(dev)
            predict(dev, f'data/SIG/sig_{version}/dev.txt.features.preproc', device=device, output_suffix='_dev_out')
    ...
    if test_pred in history and isinstance(history[test_pred], MetricDict) and not any(
        ...
        predict(checkpoint, f'data/SIG/sig_{version}/test.txt.features.preproc', device=device)
...
def post_process_then_run_eval_script(path, use_fast=True, dev=True, version='2.0'):
    try:
        ...
        scores: SmatchScores = smatch_eval(f'{path}.post',
                                           f'data/SIG/sig_{version}/{"dev" if dev else "test"}.txt',
                                           use_fast=use_fast)
