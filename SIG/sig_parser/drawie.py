...
from sig_parser.SIGGraph import SIGGraph,SIGGraph_orignal
        ...
        with open(file_path, encoding='utf-8') as f:
            for line in f:
                ...
                if line.startswith('# ::id '):
                    ...
                    mysig = SIGGraph(line)
                    mysig.visualize()
                    yield tokens, lemmas, pos_tags, ner_tags, mysig, entity_type, trigger_type
                '''
                ...
                '''
    @staticmethod
    def read_predict(file_path, do_reverse=True):
        with open(file_path, encoding='utf-8') as f:
            for line in f:
                line = line.rstrip()
...
def read_file(filename, do_reverse=True):
    ...
    for _tok, _lem, _pos, _ner, _mysig, _entity_type, _trigger_type in SIGIO.read(filename, do_reverse=do_reverse):
        token.append(_tok)
        lemma.append(_lem)
        pos.append(_pos)
        ner.append(_ner)
    ...
    return sigs, token, lemma, pos, ner, entity_type, trigger_type

def read_file_predict(filename, do_reverse=True):
    # read preprocessed sig file
    sigs = []
    for _mysig in SIGIO.read_predict(filename, do_reverse=do_reverse):
...
if __name__ == "__main__":
    sigs = read_file_predict('file_path')
    #sigs = read_file('file_path')
