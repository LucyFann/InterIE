...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, need_weights=None):
        ...
        if qkv_same:
            # self-attention
            #print("在计算self attention, qkv_same")
            q, k, v = self.in_proj_qkv(query)
        elif kv_same:
            # encoder-decoder attention
            #print("在计算encoder decoder attention kv_same")
        ...
        if need_weights:
            ...
            attn_weights: torch.Tensor = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)

            if need_weights == 'max':
                attn_weights, _ = attn_weights.max(dim=1)
            elif need_weights == "one":
                attn_weights = attn_weights[:, 0, :, :]
            elif need_weights == 'all':
                pass
            else:
                assert False, "need weights?"
            if need_weights == 'all':
                attn_weights = attn_weights.permute(1, 2, 0, 3)  # tgt x bsz x src
            else:
                attn_weights = attn_weights.transpose(0, 1)  # tgt x bsz x src
        else:
            attn_weights = None
