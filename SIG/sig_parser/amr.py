...
class AMR(object):
    ...
    @staticmethod
    def parse_AMR_line(line, do_reverse=True):
        """
        ...
        """
        # Current state. It denotes the last amrnificant symbol encountered. 1 for (, 2 for :, 3 for /,
        # and 0 for start state or ')'
        # Last amrnificant symbol is ( --- start processing node name
        # Last amrnificant symbol is : --- start processing relation name
        # Last amrnificant symbol is / --- start processing node value (concept name)
        # Last amrnificant symbol is ) --- current node processing is complete
        # Note that if these symbols are inside parenthesis, they are not amrnificant symbols.
        ...
        for i, c in enumerate(line.strip()):
            ...
            if c == "\"":
                ...
                # not amrnificant symbol if inside quote
                if in_quote:
                    cur_charseq.append(c)
                    continue
                ...
                if in_quote:
                    cur_charseq.append(c)
                    continue
                # Last amrnificant symbol is "/". Now we encounter ":"
                # Example:
                # :OR (o2 / *OR*
                #    :mod (o3 / official)
                ...
                # not amrnificant symbols, so we just shift.
                cur_charseq.append(c)
