...
from sig_parser.encoder import WordEncoder, ConceptEncoder
from sig_parser.decoder import DecodeLayer
from sig_parser.transformer import Transformer, SinusoidalPositionalEmbedding, SelfAttentionMask
from sig_parser.data import ListsToTensor, ListsofStringToTensor, DUM, NIL, PAD
from sig_parser.search import Hypothesis, Beam, search_by_batch
from sig_parser.utils import move_to_device
# model = Parser()
# model(x) -> model.forward(self, x)

class Parser(nn.Module):
    def __init__(self, vocabs, word_char_dim, word_dim, pos_dim, ner_dim,
                 entity_type_dim, trigger_type_dim, concept_char_dim, concept_dim,
                 ...
                 device=0, joint_arc_concept=False, levi_graph=False):
        super(Parser, self).__init__()
        self.vocabs = vocabs
        self.word_encoder = WordEncoder(vocabs,
                                        word_char_dim, word_dim, pos_dim, ner_dim, entity_type_dim, trigger_type_dim,
        ...
        self.decoder = DecodeLayer(vocabs, inference_layers, embed_dim, ff_embed_dim, num_heads, concept_dim, rel_dim,
                                   dropout, joint_arc_concept=joint_arc_concept, levi_graph=levi_graph)
    ...
    def encode_step_with_bert(self, tok, lem, pos, ner, entity_type, trigger_type, word_char, bert_token, token_subword_index):
        bert_embed, _ = self.bert_encoder(bert_token, token_subword_index=token_subword_index)
        word_repr = self.word_encoder(word_char, tok, lem, pos, ner, entity_type, trigger_type)
    ...
    def work(self, data, beam_size, max_time_step, min_time_step=1):
        # data ['lem', 'tok', 'pos', 'ner', 'word_char', 'copy_seq', 'local_token2idx', 'local_idx2token', 'concept_in', 'concept_char_in', 'concept_out', 'rel', 'entity_type', 'entity_subtype', 'mention_type', 'trigger_type', 'bert_token', 'token_subword_index']
        with torch.no_grad():
            if self.bert_encoder is not None:
                data['entity_type'] = torch.zeros_like(data['tok'])
                data['trigger_type'] = torch.zeros_like(data['tok'])
                word_repr, word_mask, probe = self.encode_step_with_bert(data['tok'], data['lem'], data['pos'],
                                                                         data['ner'], data['entity_type'],
                                                                         data['trigger_type'], data['word_char'],
            ...
            alignment_weight = search_by_batch(self, beams, mem_dict)
        return beams, alignment_weight
    ...
    def decode_step(self, inp, state_dict, mem_dict, offset, topk):
        levi_graph = self.decoder.levi_graph
        ...
        conc_ll, arc_ll, rel_ll, alignment_weight = self.decoder(probe, snt_state, new_graph_state, snt_padding_mask, None, None,
                                               copy_seq, work=True)
        ...
        for s, t, local_vocab in zip(topk_scores.tolist(), topk_token.tolist(), local_vocabs):
            ...
            results.append(res)
        return new_state_dict, results, alignment_weight
    ...
    def forward(self, data):
        ...
        if self.bert_encoder is not None:
            word_repr, word_mask, probe = self.encode_step_with_bert(data['tok'], data['lem'], data['pos'], data['ner'],
                                                                     data['entity_type'], data['trigger_type'],
        ...
        # self.decoder.joint_arc_concept = False
        ...
        # print("data_concept", data['concept_out'].size())
        # print("data_entity", data['entity_type'].size())
        # print("probe", probe.size())
        concept_loss, arc_loss, rel_loss, alignment_weight, entity_loss, trigger_loss = self.decoder(probe, word_repr, concept_repr, word_mask, concept_mask,
                                                        attn_mask, \
                                                        data['copy_seq'], target=data['concept_out'], target_rel=data['rel'][1:],
                                                        src_entity=data['entity_type'], src_trigger=data['trigger_type'],
                                                        )
        ...
        return concept_loss.mean(), arc_loss.mean(), None if rel_loss is None else rel_loss.mean(), graph_arc_loss.mean(), entity_loss, trigger_loss