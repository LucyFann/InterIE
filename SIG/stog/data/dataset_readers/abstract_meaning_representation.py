...
from stog.data.tokenizers.bert_tokenizer import SIGBertTokenizer
...
class AbstractMeaningRepresentationDatasetReader(DatasetReader):
    ...
    def __init__(self,
        ...
        if word_splitter is not None:
            self._word_splitter = SIGBertTokenizer.from_pretrained(
    ...
    def _read(self, file_path):
        ...
        for sig in SIGIO.read(file_path):
            yield self.text_to_instance(sig)
        self.report_coverage()


    def text_to_instance(self, sig) -> Instance:
        ...
        list_data = sig.graph.get_list_data(
            sig, START_SYMBOL, END_SYMBOL, self._word_splitter, max_tgt_length)
