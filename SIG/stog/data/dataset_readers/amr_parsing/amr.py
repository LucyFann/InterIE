...
from stog.data.dataset_readers.sig_parsing.graph_repair import GraphRepair
...
penman.SIGCodec._inversions.pop('domain')
penman.SIGCodec._deinversions.pop('mod')
from penman import Triple

sig_codec = penman.SIGCodec(indent=6)
...
class SIGNode:
    ...
    def __eq__(self, other):
        if not isinstance(other, SIGNode):
            return False
    ...
    def copy(self):
        ...
        self._num_copies += 1
        copy = SIGNode(self.identifier + '_copy_{}'.format(self._num_copies), attributes, self)
        return copy
...
class SIGGraph(penman.Graph):
    ...
    def _build_extras(self):
        ...
        for edge in self.edges():
            ...
            if edge.inverted:
                source, target, relation = target, source, sig_codec.invert_relation(edge.relation)
    ...
    def remove_edge(self, x, y):
        if isinstance(x, SIGNode) and isinstance(y, SIGNode):
            self._G.remove_edge(x, y)
        if isinstance(x, SIGNode):
            x = x.identifier
        if isinstance(y, SIGNode):
            y = y.identifier
    ...
    def add_node(self, instance):
        ...
        node = SIGNode(identifier, [('instance', instance)])
    ...
    def get_list_data(self, sig, bos=None, eos=None, bert_tokenizer=None, max_tgt_length=None):
        node_list = self.get_list_node()
    ...
    def decode(cls, raw_graph_string):
        _graph = sig_codec.decode(raw_graph_string)
        return cls(_graph)
    ...
    def from_prediction(cls, prediction):
        ...
        graph = cls(graph)
        try:
            GraphRepair.do(graph)
            sig_codec.encode(graph)
