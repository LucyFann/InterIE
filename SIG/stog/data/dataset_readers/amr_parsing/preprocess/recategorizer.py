...
from stog.data.dataset_readers.sig_parsing.sig_concepts import Entity, Date, Score, Quantity, Ordinal, Polarity, Polite, URL
...
class Recategorizer:
    ...
    def recategorize_file(self, file_path):
        for i, sig in enumerate(SIGIO.read(file_path), 1):
            self.recategorize_graph(sig)
            yield sig
            if i % 1000 == 0:
                logger.info('Processed {} examples.'.format(i))
        logger.info('Done.\n')

    def recategorize_graph(self, sig):
        sig.stems = [self.stemmer(l) for l in sig.lemmas]
        self.resolve_name_node_reentrancy(sig)
        self.recategorize_name_nodes(sig)
        if self.build_utils:
            return
        self.remove_wiki(sig)
        self.remove_negation(sig)
        self.recategorize_date_nodes(sig)
        self.recategorize_score_nodes(sig)
        self.recategorize_ordinal_nodes(sig)
        self.recategorize_quantities(sig)
        self.recategorize_urls(sig)

    def resolve_name_node_reentrancy(self, sig):
        graph = sig.graph
    ...
    def recategorize_name_nodes(self, sig):
        ...
        for node, _, _ in graph.get_list_node(replace_copy=False):
            ...
            if graph.is_name_node(node):
                ...
                sig_type = sig.graph.get_name_node_type(node)
                backup_ner_type = self._map_name_node_type(sig_type)
                entity = Entity.get_aligned_entity(
                    node, sig, backup_ner_type, self.entity_type_cooccur_counter)
                if len(entity.span):
                    self.recat_named_entity_count += 1
                entities.append(entity)
        entities, removed_entities = resolve_conflict_entities(entities)
        if not self.build_utils:
            type_counter = Entity.collapse_name_nodes(entities, sig)
            for entity in removed_entities:
                sig_type = sig.graph.get_name_node_type(entity.node)
                backup_ner_type = self._map_name_node_type(sig_type)
                entity = Entity.get_aligned_entity(
                    entity.node, sig, backup_ner_type, self.entity_type_cooccur_counter)
                Entity.collapse_name_nodes([entity], sig, type_counter)
        else:
            self._update_utils(entities, sig)

    def recategorize_date_nodes(self, sig):
        graph = sig.graph
        dates = []
        for node, _, _ in graph.get_list_node(replace_copy=False):
            if node.copy_of is not None:
            ...
            if graph.is_date_node(node) and Date.collapsable(node, graph):
                ...
                date = self._get_aligned_date(node, sig)
                if date.span is not None:
                    self.recat_date_entity_count += 1
                dates.append(date)
        dates, removed_dates = resolve_conflict_entities(dates)
        Date.collapse_date_nodes(dates, sig)

    def recategorize_score_nodes(self, sig):
        graph = sig.graph
        scores = []
        for node, _, _ in graph.get_list_node(replace_copy=False):
            if node.copy_of is not None:
            ...
            if node.instance == 'score-entity':
                ...
                score = Score(node, sig)
                if score.span is not None:
                    self.recat_score_entity_count += 1
                scores.append(score)
        Score.collapse_score_nodes(scores, sig)

    def recategorize_ordinal_nodes(self, sig):
        graph = sig.graph
        ordinals = []
        for node, _, _ in graph.get_list_node(replace_copy=False):
            if node.copy_of is not None:
            ...
            if node.instance == 'ordinal-entity':
                ...
                ordinal = Ordinal(node, sig)
                if ordinal.span is not None:
                    self.recat_ordinal_entity_count += 1
                ordinals.append(ordinal)
        Ordinal.collapse_ordinal_nodes(ordinals, sig)

    def recategorize_quantities(self, sig):
        quantity = Quantity(sig)
        self.recat_quantity_count += quantity.abstract()
        self.quantity_count += quantity.quant_count

    def recategorize_urls(self, sig):
        url = URL(sig)
        try:
    ...
    def _get_aligned_date(self, node, sig):
        date = Date(node, sig.graph)
        if len(date.attributes) + len(date.edges) == 0:
            return date
        alignment = date._get_alignment(sig)
        date._get_span(alignment, sig)
        return date

    def _update_utils(self, entities, sig):
        if not self.build_entity_map:
            for entity in entities:
                wiki_title = sig.graph.get_name_node_wiki(entity.node)
                if wiki_title is None:
                    wiki_title = '-'
                for text_span in entity.get_text_spans(sig):
                ...
                if len(entity.span) == 0:
                    continue
                entity_text = ' '.join(sig.tokens[index] for index in entity.span).lower()
            ...
            for entity in entities:
                ...
                if entity.ner_type != Entity.unknown_entity_type:
                    self.name_type_cooccur_counter[entity.sig_type][entity.ner_type] += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sig_train_file')
    parser.add_argument('--sig_files', nargs='+')
    ...
    recategorizer = Recategorizer(
        train_data=args.sig_train_file,
    ...
    for file_path in args.sig_files:
        with open(file_path + '.recategorize', 'w', encoding='utf-8') as f:
            for sig in recategorizer.recategorize_file(file_path):
                f.write(str(sig) + '\n\n')
