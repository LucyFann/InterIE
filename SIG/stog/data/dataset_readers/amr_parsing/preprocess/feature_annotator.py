...
if __name__ == '__main__':
    ...
    # annotator = FeatureAnnotator('http://localhost:8888', args.compound_file)
    nlp = stanza.Pipeline(lang='en', processors='tokenize, pos, lemma, ner', tokenize_pretokenized=True)
    file_path = 'data/SIG/ace_sig/train.txt'
    logger.info('Processing {}'.format(file_path))
    with open(file_path + '.features', 'w', encoding='utf-8') as f, open(file_path + '.tok', 'r', encoding='utf-8') as g:
        for i, sig in enumerate(SIGIO.read(file_path), 1):
            if i % 1000 == 0:
                logger.info('{} processed.'.format(i))
            tok = g.readline()
            # 转为列表
            tok = ast.literal_eval(tok[11:-1])
            sent = nlp([tok]).sentences[0]
            # 把sent中的数据依次取出来
            sent = sent.to_dict()
            lem, pos, ner = [], [], []
            for x in sent:
                ...
                pos.append(x['xpos'])
                ner.append(x['ner'])
            # 现在的token改成oneie中的token和对应的lemma pos ner
            sig.tokens = tok
            sig.lemmas = lem
            sig.pos_tags = pos
            sig.ner_tags = ner
