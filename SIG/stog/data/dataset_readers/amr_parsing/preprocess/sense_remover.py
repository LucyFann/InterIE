...
class SenseRemover:
    """
    Remove sense numbers of SIG node instances
    """
    ...
    def remove_file(self, file_path):
        for i, sig in enumerate(SIGIO.read(file_path), 1):
            ...
            self.remove_graph(sig)
            yield sig

    def remove_graph(self, sig):
        graph = sig.graph
        for node in graph.get_nodes():
            ...
            lemma = self.find_corresponding_lemma(instance, lemmas, sig)
            if lemma is None:
    ...
    def find_corresponding_lemma(self, instance, lemmas, sig):
        self.sig_instance_count += 1
        input_lemma = None
        for lemma in lemmas:
            if lemma in sig.lemmas:
                input_lemma = lemma
                break
    ...
    def print_statistics(self):
        logger.info('sense remove rate: {}% ({}/{})'.format(
            self.removed_instance_count / self.sig_instance_count,
            self.removed_instance_count, self.sig_instance_count))
        logger.info('restore rate: {}% ({}/{})'.format(
            self.restore_count / self.sig_instance_count,
            self.restore_count, self.sig_instance_count))
...
if __name__ == '__main__':
    ...
    for file_path in args.sig_files:
        with open(file_path + '.nosense', 'w', encoding='utf-8') as f:
            for sig in remover.remove_file(file_path):
                f.write(str(sig) + '\n\n')
