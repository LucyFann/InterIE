...
class Alignment:
    ...
    def __init__(self, node, url, sig, indexes, score):
        self.node = node
        self.url = url
        self.sig = sig
    ...
    def __str__(self):
        return 'node: {}\nurl: {}\naligned_token_indexes: {}\naligned_tokens: {}'.format(
            ...
            ' '.join([self.sig.lemmas[i] for i in self.aligned_token_indexes]))
...
class URL:
    ...
    def fix_url_node(self, node):
        name_node = list(self.sig.graph._G[node].items())[0][0]
        url = name_node.ops[0]
        self.sig.graph.remove_edge(node, name_node)
        self.sig.graph.remove_subtree(name_node)
        self.sig.graph.add_node_attribute(node, 'value', url)
        return url

    def align_url(self, node, url):
        candidate_alignments = []
        for index in range(len(self.sig.tokens)):
            score = self.maybe_align(index, url)
            if score > 0:
                alignment = Alignment(node, url, self.sig, [index], score)
    ...
    def abstract_url(self):
        ...
        for i, alignment in enumerate(self.alignments):
            ...
            offset += len(span) - 1
            self.sig.abstract_map[abstract] = dict(
                type='url-entity',
                span=' '.join(map(self.sig.lemmas.__getitem__, span)),
                value=alignment.url)
            self.sig.replace_span(span, [abstract], ['NN'], ['URL'])
            self.sig.graph.replace_node_attribute(alignment.node, 'value', alignment.url, abstract)
        return count

    def remove_redundant_url(self):
        while len(self.sig.lemmas)>1:
            for i in range(len(self.sig.lemmas)):
                lemma = self.sig.lemmas[i]
                if re.search('(https?:|<a.*href=|^</a>$)', lemma):
                    self.sig.remove_span([i])
                    break
            else:
                break
