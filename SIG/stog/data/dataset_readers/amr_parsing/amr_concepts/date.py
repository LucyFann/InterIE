...
class Date:
    ...
    @staticmethod
    def collapse_date_nodes(dates, sig):
        if sig.abstract_map is None:
            sig.abstract_map = {}
        ...
        for date in dates:
            if date.span:
                ...
                sig.abstract_map[abstract] = Date.save_collapsed_date_node(
                    date, span_with_offset, sig)
                sig.replace_span(span_with_offset, [abstract], ['NNP'], [date.ner_type])
                # Remove edges
                for source, target in list(sig.graph._G.edges(date.node)):
                    edge_label = sig.graph._G[source][target]['label']
                    if edge_label in Date.edge_list:
                        sig.graph.remove_edge(source, target)
                        sig.graph.remove_subtree(target)
                # Update instance
                sig.graph.replace_node_attribute(date.node, 'instance', 'date-entity', abstract)
                # Remove attributes
                for attr, value in date.attributes.items():
                    sig.graph.remove_node_attribute(date.node, attr, value)
    ...
    @staticmethod
    def save_collapsed_date_node(date, span, sig):
        return dict(
            type='date-entity',
    ...
    def _get_alignment(self, sig):
        alignment = defaultdict(list)
        for item in list(self.attributes.items()) + list(self.edges.items()):
            attr, value = item
            for i in range(len(sig.tokens)):
                confidence = self._maybe_align(attr, value, i, sig)
                if confidence != 0:
                    alignment[i].append((item, confidence))
        return alignment

    def _get_span(self, alignment, sig):
        spans = self.group_indexes_to_spans(alignment.keys(), sig)
        span_scores = []
        for span in spans:
            attr_set = set()
            for index in span:
    ...
    def _clean_span(self, spans, alignment, sig):
        ...
        for span in spans:
            ...
            _spans = self.group_indexes_to_spans(indexes, sig)
            clean_spans.append(max(_spans, key=lambda s: len(s)))
        return clean_spans

    def _maybe_align(self, attr, value, index, sig):
        if attr == 'year':
            return self._maybe_align_year(value, index, sig)
        elif attr == 'month':
            return self._maybe_align_month(value, index, sig)
        elif attr == 'day':
            return self._maybe_align_day(value, index, sig)
        elif attr == 'decade':
            return self._maybe_align_decade(value, index, sig)
        elif attr == 'time':
            return self._maybe_align_time(value, index, sig)
        elif attr == 'century':
            return self._maybe_align_century(value, index, sig)
        elif attr == 'era':
            return self._maybe_align_era(value, index, sig)
        elif attr == 'quant':
            return self._maybe_align_quant(value, index, sig)
        elif attr == 'quarter':
            return self._maybe_align_quarter(value, index, sig)
        elif attr == 'weekday':
            return self._maybe_align_weekday(value, index, sig)
        else:
            return self._maybe_align_basic(value, index, sig)

    def _maybe_align_basic(self, value, index, sig):
        value = str(value).lower()
        if re.search(r'^".*"$', value):
            value = value[1:-1]
        if sig.tokens[index].lower() == value or sig.lemmas[index].lower() == value:
            return 10
        stripped_lemma = self._strip_date_lemma(sig.lemmas[index])
    ...
    def _maybe_align_year(self, value, index, sig):
        basic_confidence = self._maybe_align_basic(value, index, sig)
    ...
    def _maybe_align_month(self, value, index, sig):
        lemma = sig.lemmas[index]
        if 0 < value < 13:
    ...
    def _maybe_align_day(self, value, index, sig):
        basic_confidence = self._maybe_align_basic(value, index, sig)
        if basic_confidence != 0:
            return basic_confidence
        lemma = sig.lemmas[index]
    ...
    def _maybe_align_decade(self, value, index, sig):
        basic_confidence = self._maybe_align_basic(value, index, sig)
        if basic_confidence != 0:
            return basic_confidence
        year_confidence = self._maybe_align_year(value, index, sig)
        if year_confidence != 0:
            return year_confidence
        try:
            w2n_number = str(w2n.word_to_num(sig.lemmas[index]))
        except:
            w2n_number = None
    ...
    def _maybe_align_time(self, value, index, sig):
    ...
    def _maybe_align_century(self, value, index, sig):
        value = str(value)
        basic_confidence = self._maybe_align_basic(value, index, sig)
        if basic_confidence != 0:
            return basic_confidence
    ...
    def _maybe_align_era(self, value, index, sig):
        value = str(value)
        basic_confidence = self._maybe_align_basic(value, index, sig)
        if basic_confidence != 0:
            return basic_confidence
        lemma = sig.lemmas[index].replace('.', '')
        if re.search(r'^".*"$', value):
            value = value[1:-1]
    ...
    def _maybe_align_quant(self, value, index, sig):
        value = str(value)
        basic_confidence = self._maybe_align_basic(value, index, sig)
        if basic_confidence != 0:
            return basic_confidence
    ...
    def _maybe_align_quarter(self, value, index, sig):
        value = str(value)
        basic_confidence = self._maybe_align_basic(value, index, sig)
        if basic_confidence != 0:
            return basic_confidence
    ...
    def _maybe_align_weekday(self, value, index, sig):
        value = str(value).lower()
        basic_confidence = self._maybe_align_basic(value, index, sig)
        if basic_confidence != 0:
            return basic_confidence
        lemma = sig.lemmas[index].lower()
        if value.startswith(lemma) or lemma.startswith(value) or lemma.endswith(value):
            return 10
        return 0

    def group_indexes_to_spans(self, indexes, sig):
        ...
        for idx in indexes:
            if last_index is None or idx - last_index > 3:
                ...
                for i in range(last_index + 1, idx):
                    if re.search(r"(,|'s|of|'|-|in|at|on|about|the|every|\(|\))", sig.tokens[idx - 1]):
                        continue
                    else:
                        spans.append([])
                        break
