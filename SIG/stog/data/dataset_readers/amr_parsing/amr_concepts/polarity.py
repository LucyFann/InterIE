...
class Polarity:
    ...
    def __init__(self, sig, dry=False):
        self.sig = sig
        self.dry = dry
    ...
    def remove_polarity(self):
        ...
        for node in self.get_negated_nodes():
            for attr, value in node.attributes:
                if attr == 'polarity':
                    if not self.dry:
                        self.sig.graph.remove_node_attribute(node, attr, value)
                    count += 1
        for node in self.sig.graph.get_nodes():
    ...
    def predict_polarity(self):
        ...
        """
        for i in range(len(self.sig.tokens)):
    ...
    def restore_polarity(self):
        ...
        remaining_nodes = list(self.sig.graph.get_nodes())
        ...
        for neg_index, head in special_negations:
            head_lemma = self.sig.lemmas[neg_index]
            for instance, nodes in instance_map.items():
    ...
    def special_restoration(self, neg_index, head, node):
        head_lemma = self.sig.lemmas[head] if isinstance(head, int) else head
        if node.instance == 'have-polarity':
            if self.dry:
                for attr, value in node.attributes:
                    if attr == 'ARG2' and value == '-':
        ...
        if head_lemma == 'face' and node.instance == 'face':
            for source, target in self.sig.graph._G.in_edges(node):
                if source.instance == 'want':
                    if self.dry:
                        ...
                        self.sig.graph.add_node_attribute(source, 'polarity', '-')
                    break
            return True
        return False

    def is_negation(self, index):
        lemma = self.sig.lemmas[index]
        next_lemma = self.sig.lemmas[index + 1] if index + 1 < len(self.sig.lemmas) else None
    ...
    def add_special_negation(self, index):
        lemma = self.sig.lemmas[index]
        last_lemma = self.sig.lemmas[index - 1] if index - 1 >= 0 else None
    ...
    def is_head(self, index):
        lemma = self.sig.lemmas[index]
        token = self.sig.tokens[index]
        pos_tag = self.sig.pos_tags[index]
        next_lemma = self.sig.lemmas[index + 1] if index + 1 < len(self.sig.lemmas) else None
    ...
    def is_false_positive(self, index, head):
        ...
        last_lemma = self.sig.lemmas[index - 1] if index - 1 >= 0 else None
        next_lemma = self.sig.lemmas[index + 1] if index + 1 < len(self.sig.lemmas) else None
        third_lemma = self.sig.lemmas[index + 2] if index + 2 < len(self.sig.lemmas) else None
        head_lemma = self.sig.lemmas[head]
        if next_lemma and next_lemma in ('matter', 'address', 'other', 'only', ',', '.'):
            return True
        if next_lemma == '-' and third_lemma != 'alignment':
            return True
        if next_lemma == 'like' and self.sig.pos_tags[index + 1] == 'IN':
            return True
    ...
    def get_special_head(self, index):
        lemma = self.sig.lemmas[index]
        last_lemma = self.sig.lemmas[index - 1] if index - 1 >= 0 else None
        llast_lemma = self.sig.lemmas[index - 2] if index - 2 >= 0 else None
        next_lemma = self.sig.lemmas[index + 1] if index + 1 < len(self.sig.lemmas) else None
        third_lemma = self.sig.lemmas[index + 2] if index + 2 < len(self.sig.lemmas) else None
        fourth_lemma = self.sig.lemmas[index + 3] if index + 3 < len(self.sig.lemmas) else None
        if lemma in ('cant',):
            return index
        ...
        if next_lemma in ('as',):
            if third_lemma in ('much', 'severely'):
                return index + 2
            if third_lemma and self.sig.pos_tags[index + 2] in ('RB',):
                return None
            return index + 1
    ...
    def sort_node_by_distance(self, head, nodes):
        if len(nodes) <= 1:
            return nodes
        lemmas = self.sig.lemmas
        scores = []
        alignment = {}
        for node in nodes:
            aligned_indexes = set()
            for n in self.sig.graph.get_subtree(node, max_depth=1)[1:]:
                if n.instance in lemmas:
                    aligned_indexes.add(lemmas.index(n.instance))
            if len(aligned_indexes) == 0:
                for n in self.sig.graph.get_subtree(node, max_depth=5)[1:]:
                    if n.instance in lemmas:
                        aligned_indexes.add(lemmas.index(n.instance))
            for n, _ in self.sig.graph._G.in_edges(node):
                if n.instance in lemmas:
                    aligned_indexes.add(lemmas.index(n.instance))
    ...
    def get_precision(self):
        ...
        rest_nodes = list(self.sig.graph.get_nodes())
        ...
        for index, head in self.special_negations:
            match, correct = False, False
            head_lemma = self.sig.lemmas[index]
            for instance, nodes in instances.items():
                if head_lemma == instance:
                    continue
                if self.is_match(head, instance):
    ...
    def is_match(self, index, instance):
        ...
        lemma = self.sig.lemmas[index] if isinstance(index, int) else index
