...
class AlignedPairs:
    ...
    def __init__(self, quant_tokens, x, y, sig, score=0, near=-float('inf')):
    ...
    def __str__(self):
        return '{}: {}({})'.format(
            ...
            self.sig.tokens[self.snt_token_index],
...
class Quantity:
    ...
    def __init__(self, sig, dry=False):
        ...
        self.dry = dry
        self.alignments = []
        self.ordered_node_list = [n for n, _, _ in sig.graph.get_list_node()]
        self.quant_count = 0
    ...
    def abstract_group(self, groups):
        ...
        for i, alignment in enumerate(representatives):
            ...
            pos_tag = self.sig.pos_tags[span[0]]
            if pos_tag in ('0', 'O'):
                pos_tag = 'CD'
            self.sig.replace_span(span, [abstract], [pos_tag], ['NUMBER'])
            for a in groups[i]:
                count += 1
                self.sig.graph.replace_node_attribute(a.node, a.attr, a.value, abstract)#
        return count

    def get_alignment(self, tokens, node_position, node, attr, value):
        candidate_alignments = []
        for start in range(len(self.sig.tokens) - len(tokens) + 1):
            ...
            for i, index in enumerate(range(start, start + len(tokens))):
                ...
                if score > 0:
                    alignment.aligned_pairs.append(
                        AlignedPairs(tokens, i, index, self.sig, score, near))
    ...
    def get_node_position(self, node):
        ...
        if position == -1:
            for _, child in self.sig.graph._G.edges(node):
                if self.sig.graph._G[node][child]['label'] == 'unit':
    ...
    def maybe_align(self, index, token):
        lemma = self.sig.lemmas[index].replace(',', '').lower()
        if lemma == token:
            return 10
