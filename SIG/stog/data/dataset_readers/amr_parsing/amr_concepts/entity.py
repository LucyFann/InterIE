...
def group_indexes_to_spans(indexes, sig):
    ...
    for idx in indexes:
        if last_index is None or idx - last_index > 2:
            ...
            if re.search(r"(,|'s|of|'|-|in)", sig.tokens[idx - 1]):
...
class Entity:
    ...
    def __init__(self, span=None, node=None, ner_type=None, sig_type=None, confidence=0, alignment=None):
        self.span = span
        self.node = node
    ...
    def get_text_spans(self, sig):
        spans = []
        span = []
        for index in self.span:
            span.append(sig.tokens[index])
    ...
    @classmethod
    def get_aligned_entity(cls, node, sig, backup_ner_type, entity_type_lut):
        if len(node.ops) == 0:
            return None
        alignment = cls.get_alignment_for_ops(rephrase_ops(node.ops), sig)
        if len(alignment) == 0:
            alignment = cls.get_alignment_for_ops(node.ops, sig)
        entity1 = cls(node=node, alignment=alignment)
        entity1._get_aligned_info(sig, backup_ner_type, entity_type_lut)

        alignment = cls.get_alignment_for_ops(tokenize_ops(node.ops), sig)
        entity2 = cls(node=node, alignment=alignment)
        entity2._get_aligned_info(sig, backup_ner_type, entity_type_lut)
    ...
    @staticmethod
    def get_alignment_for_ops(ops, sig):
        alignment = {}
        for i, op in enumerate(ops):
            for j, token in enumerate(sig.tokens):
                confidence = Entity.maybe_align_op_to(op, j, sig)
                if confidence > 0:
    ...
    @staticmethod
    def maybe_align_op_to(op, index, sig):
        if not isinstance(op, str):
            op = str(op)
        if re.search(r'^".*"$', op):
            op = op[1:-1]
        op_lower = op.lower()
        token_lower = sig.tokens[index].lower()
        lemma_lower = sig.lemmas[index].lower()
        stripped_lemma_lower = strip_lemma(lemma_lower)
        # Exact match.
        if sig.tokens[index] == op or sig.lemmas[index] == op:
            return 15
        elif op_lower == token_lower or op_lower == lemma_lower or op_lower == stripped_lemma_lower:
            return 10
        # Stem exact match.
        elif STEMMER(op) == sig.stems[index]:
            return 8
        # Tagged as named entity and match the first 3 chars.
        elif sig.is_named_entity(index) and (
        ...
        # Match after mapping.
            ...
            return max(Entity.maybe_align_op_to(mapped_op, index, sig) for mapped_op in Entity.entity_map[op])
        else:
            return 0

    @staticmethod
    def collapse_name_nodes(entities, sig, type_counter=None):
        ...
        for entity in entities:
            if len(entity.span) > 0:
                ...
                sig.abstract_map[abstract] = Entity.save_collapsed_name_node(
                    entity, span_with_offset, sig)
                sig.replace_span(span_with_offset, [abstract], ['NNP'], [entity.ner_type])
                sig.stems = sig.stems[:span_with_offset[0]] + [abstract] + sig.stems[span_with_offset[-1] + 1:]
                sig.graph.remove_node_ops(entity.node)
                sig.graph.replace_node_attribute(
                    entity.node, 'instance', entity.node.instance, abstract)
                offset += len(entity.span) - 1
            else:
                sig.graph.remove_node(entity.node)
        return type_counter

    @staticmethod
    def save_collapsed_name_node(entity, span, sig):
        return dict(
            type='named-entity',
            span=' '.join(map(sig.tokens.__getitem__, span)),
            ops=' '.join(entity.get_ops())
        )

    def _get_aligned_info(self, sig, backup_ner_type, entity_type_lut):
        spans = group_indexes_to_spans(self.alignment.keys(), sig)
        spans.sort(key=lambda span: len(span), reverse=True)
        spans = self._clean_span(spans, sig)
        sig_type = sig.graph.get_name_node_type(self.node)
        candidate_spans = []
        for span in spans:
        ...
        if len(candidate_spans):
            ...
            entity_mention = ' '.join([sig.tokens[index] for index in best_span]).lower()
    ...
    def _clean_span(self, spans, sig):
