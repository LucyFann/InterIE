...
class Ordinal:
    ...
    def _get_alignment(self):
        ...
        for i, op in enumerate(self.ops):
            if re.search(r'^".*"$', op):
                op = op[1: -1]
            for j in range(len(self.sig.tokens)):
    ...
    def _maybe_align(self, op, index):
        lemma = self.sig.lemmas[index].lower().replace(',', '')
        if op == lemma:
            return 10
    ...
    @staticmethod
    def collapse_ordinal_nodes(ordinals, sig):
        node_count = 0
        ...
        for ordinal in ordinals:
            if ordinal.span is not None:
                ...
                sig.abstract_map[abstract] = ordinal.to_dict(sig, span)
                sig.replace_span(span, [abstract], ['JJ'], [ordinal.ner_type])
                sig.stems = sig.stems[:span[0]] + [abstract] + sig.stems[span[-1] + 1:]
                for attr, value in ordinal.node.attributes:
                    if attr == 'value':
                        sig.graph.remove_node_attribute(ordinal.node, attr, value)
                        break
                ...
                if ordinal.value_node:
                    ...
                    sig.graph.remove_edge(ordinal.node, ordinal.value_node)
                    sig.graph.remove_subtree(ordinal.value_node)
                offset += len(ordinal.span) - 1
            else:
                edges = list(sig.graph._G.in_edges(ordinal.node))
                for source, target in edges:
