...
class Polite:
    ...
    def predict_polite(self):
        for i in range(len(self.sig.tokens)):
            if self.sig.lemmas[i] == 'please':
                if self.sig.lemmas[i + 1: i + 3] == ['take', 'a']:
                    self.heads.append((i, i + 3))
                elif i - 2 >= 0 and self.sig.lemmas[i - 2] == 'can':
                    self.heads.append((i, i - 2))
                elif i+1 <len(self.sig.tokens):
    ...
    def restore_node_polite(self, node):
        if self.dry:
            ...
            self.sig.graph.add_node_attribute(node, 'polite', '+')
