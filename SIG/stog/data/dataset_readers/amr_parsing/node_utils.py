...
from stog.data.dataset_readers.sig_parsing.propbank_reader import PropbankReader
...
class NodeUtilities:
    ...
    @classmethod
    def from_raw(cls, sig_train_files, propbank_dir, verbalization_file, dump_dir,
                 # Hyperparameters
        ...
        senseless_node_counter = cls._get_senseless_node_counter(sig_train_files)
        ...
        nu._update_counter_from_train_files(sig_train_files, train_file_base_freq)
    ...
    @staticmethod
    def _get_senseless_node_counter(sig_train_files):
        logger.info('Building the senseless node counter.')
        sense_less_nodes = []
        for sig_file in sig_train_files:
            for sig in SIGIO.read(sig_file):
                for node in sig.graph.get_nodes():
    ...
    def _update_counter_from_train_files(self, sig_train_files, base_freq=1):
        logger.info('Updating (lemma, frame) counter from SIG train files.')
        for file_path in sig_train_files:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sig_train_files', nargs='+', default=['data/all_sig/train_sig.txt.features'])
    parser.add_argument('--propbank_dir', default='data/SIG/sig_annotation_3.0/data/frames/propbank-sig-frames-xml-2018-01-25')
    parser.add_argument('--verbalization_file', default='data/SIG/verbalization-list-v1.06.txt')
    parser.add_argument('--dump_dir', default='sig_3.0_utils')
