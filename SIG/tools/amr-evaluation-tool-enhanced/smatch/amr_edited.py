...
class SIG(object):
    ...
    """
    def __init__(self, node_list=None, node_value_list=None, relation_list=None, attribute_list=None, reent=None, allrelations=None):
        """
        ...
        """
        # initialize SIG graph nodes using list of nodes name
    ...
    @staticmethod
    def parse_SIG_line(line, normalize_inv = True):
        """
	...
	reent = []
        ...
        for i, c in enumerate(line.strip()):
		    ...
		    if relation_name.endswith("-of") and normalize_inv:
                ...
                if state == 1:
                    ...
                    if node_name in node_dict:
                        print >> ERROR_LOG, "Duplicate node name ", node_name, " in parsing SIG"
                        return None
			    ...
			    allrelations.append((node_name,cur_relation_name[:-3], stack[-2]))
                    ...
                    print >> ERROR_LOG, "Error in parsing SIG", line[0:i+1]
                    return None
                state = 3
            elif c == ")":
                if in_quote:
		    ...
		    reent.append((stack[-1],relation_name,relation_value))
                ...
                cur_charseq.append(c)
        #create data structures to initialize an SIG
        node_value_list = []
        relation_list = []
        attribute_list = []
        ...
        result_sig = SIG(node_name_list, node_value_list, relation_list, attribute_list, reent, allrelations)
        return result_sig
...
if __name__ == "__main__":
    ...
    for line in open(sys.argv[1]):
        ...
        if cur_line == "" or cur_line.startswith("#"):
            continue
        print >> DEBUG_LOG, "SIG", sig_count
        current = SIG.parse_SIG_line(cur_line)
        current.output_sig()
        sig_count += 1
