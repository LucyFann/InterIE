...
# change if needed
isi_dir_pre = "/nfs/web/isi.edu/cgi-bin/div3/mt/save-sig"
...
def get_names(file_dir, files):
    ...
    Args:
    file_dir: SIG file folder
    files: a list of SIG names, e.g. nw_wsj_0001_1
...
def compute_files(user1, user2, file_list, dir_pre, start_num):
    ...
    for fi in file_list:
        ...
        try:
            ...
            break
        cur_sig1 = smatch.get_sig_line(file1_h)
        cur_sig2 = smatch.get_sig_line(file2_h)
        if cur_sig1 == "":
            print >> ERROR_LOG, "SIG 1 is empty"
            continue
        if cur_sig2 == "":
            print >> ERROR_LOG, "SIG 2 is empty"
            continue
        sig1 = sig.SIG.parse_SIG_line(cur_sig1)
        sig2 = sig.SIG.parse_SIG_line(cur_sig2)
        test_label = "a"
        gold_label = "b"
        sig1.rename_node(test_label)
        sig2.rename_node(gold_label)
        (test_inst, test_rel1, test_rel2) = sig1.get_triples()
        (gold_inst, gold_rel1, gold_rel2) = sig2.get_triples()
        if verbose:
...
def build_arg_parser():
    ...
    parser.add_argument("--fl", type=argparse.FileType('r'), help='SIG ID list file')
    parser.add_argument('-f', nargs='+', help='SIG IDs (at least one)')
    parser.add_argument("-p", nargs='*', help="User list (can be none)")
    parser.add_argument("--fd", default=isi_dir_pre, help="SIG File directory. Default=location on isi machine")
...
def build_arg_parser2():
    ...
    parser.add_option("--fl", dest="fl", type="string", help='SIG ID list file')
    parser.add_option("-f", dest="f", type="string", action="callback", callback=cb, help="SIG IDs (at least one)")
...
def check_args(args):
    ...
    if args.fl is not None:
        ...
        sig_ids = file_line.strip().split()
    elif args.f is None:
        print >> ERROR_LOG, "No SIG ID was given"
        return [], [], False
    else:
        sig_ids = args.f
    names = []
    check_name = True
    if args.p is None:
        names = get_names(args.fd, sig_ids)
        # no need to check names
        check_name = False
        if len(names) == 0:
            print >> ERROR_LOG, "Cannot find any user who tagged these SIG"
            return [], [], False
        else:
            names = args.p
    if len(names) == 0:
    ...
    # check if all the SIG_id and user combinations are valid
    if check_name:
        pop_name = []
        for i, name in enumerate(names):
            for sig in sig_ids:
                sig_path = args.fd + name + "/" + sig + ".txt"
                if not os.path.exists(sig_path):
                    print >> ERROR_LOG, "User", name, "fails to tag SIG", sig
                    pop_name.append(i)
                    break
        if len(pop_name) != 0:
            pop_num = 0
            for p in pop_name:
                ...
                pop_num += 1
        if len(names) < 2:
            print >> ERROR_LOG, "Not enough users to evaluate. Smatch requires >2 users who tag all the SIGs"
            return "", "", False
    return sig_ids, names, True
