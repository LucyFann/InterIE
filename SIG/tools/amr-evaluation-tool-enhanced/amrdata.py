...
'''
SIGDataset reads the file generated by preprocessing.sh and it generates a SIGSentence instance for each sentence,
...
import sys
import smatch.sig_edited as sigannot
sys.path.append("..")
reload(sys)
...
class SIGDataset:
    def _var2concept(self, sig):
        v2c = {}
        for n, v in zip(sig.nodes, sig.node_values):
            v2c[n] = v
        return v2c

    def __init__(self, prefix, sigs, normalize = True):
        ...
        if sigs:
            ...
            for graph, alignments, depslines, tokens, pos, lemmas, nes in zip(allgraphs, allalignments, alldepslines, alltokens, allpos, alllemmas, allnes):
                graph = graph.strip()
                sig = sigannot.SIG.parse_SIG_line(graph.replace("\n",""), False)
                variables = {}
                for n, v in zip(sig.nodes, sig.node_values):
                    variables[n] = v
		role_triples = sig.get_triples3()
                relations = []
                ...
                self.sentences.append(SIGSentence(tokens, pos, lemmas, nes, dependencies, variables, relations, graph, alignments))
        else:
                ...
                for line in depslines.split("\n"):
                    ...
                    if regex is not None:
                        ...
                        if a == -1:
                            ...
                            dependencies.append((a, label, b))
                self.sentences.append(SIGSentence(tokens, pos, lemmas, nes, dependencies))
