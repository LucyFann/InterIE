...
par = 0
k = -1
sents = SIGDataset(prefix, True, False).getAllSents()
fsig = open("np_graphs.txt","w")
...
while True:
	...
	for n in nps:
		...
		if nodes == []: # no alignments
			continue
		v2c = defaultdict(str)
		sig_annot = sig.SIG.parse_SIG_line(sents[k].graph.replace("\n",""))
		for key in var2concept(sig_annot):
			v2c[str(key)] = str(var2concept(sig_annot)[key])
...
#				rels.insert(0, ("TOP", ":top", node))
		...
		fsig.write(to_string(rels2, rels2[0][0])[0] + "\n")
		fsent.write(" ".join(n) + "\n")
		...
		#if rels2 != [] and str(src.sig.SIG.triples2String(rels2)).startswith("("):
		#	print "# ::snt", " ".join(n)
		#	graph = src.sig.SIG.triples2String(rels2)
		#	print graph
		#	print ""
