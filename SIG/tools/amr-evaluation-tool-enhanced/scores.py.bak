...
'''
Computes SIG scores for concept identification, named entity recognition, wikification,
...
def var2concept(sig):
    v2c = {}
    for n, v in zip(sig.nodes, sig.node_values):
        v2c[n] = v
    return v2c
...
k = 0
for sig_pred, sig_gold in zip(pred, gold):
    sig_pred = sig.SIG.parse_SIG_line(sig_pred.replace("\n","")) 
    dict_pred = var2concept(sig_pred)
    triples_pred = [t for t in sig_pred.get_triples()[1]]
    triples_pred.extend([t for t in sig_pred.get_triples()[2]])
    sig_gold = sig.SIG.parse_SIG_line(sig_gold.replace("\n",""))
    dict_gold = var2concept(sig_gold)
    triples_gold = [t for t in sig_gold.get_triples()[1]]
    triples_gold.extend([t for t in sig_gold.get_triples()[2]])
