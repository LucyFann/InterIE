...
"""
This script computes smatch score between two SIGs.
For detailed description of smatch, see http://www.isi.edu/natural-language/sig/smatch-13.pdf
...
# Default true (compute a single score for all SIGs in two files)
single_score = True
...
def get_sig_line(input_f):
    ...
    for line in input_f:
        ...
        if line == "":
            if not has_content:
                # empty lines before current SIG
                continue
            else:
                # end of current SIG
                break
        if line.strip().startswith("#"):
            # ignore the comment line (starting with "#") in the SIG file
            continue
        else:
            has_content = True
            cur_sig.append(line.strip())
    return "".join(cur_sig)
...
def build_arg_parser():
    ...
    parser.add_argument('-f', nargs=2, required=True, type=argparse.FileType('r'),
                        help='Two files containing SIG pairs. SIGs in each file are separated by a single blank line')
    ...
    parser.add_argument('--ms', action='store_true', default=False,
                        help='Output multiple scores (one SIG pair a score)' \
...
def build_arg_parser2():
    ...
    parser.add_option("-f", "--files", nargs=2, dest="f", type="string",
                      help='Two files containing SIG pairs. SIGs in each file are ' \
    ...
    parser.add_option("--ms", "--multiple_score", action='store_true', dest="ms",
                      help='Output multiple scores (one SIG pair a score) instead of ' \
...
def get_best_match(instance1, attribute1, relation1,
    ...
    Arguments:
        instance1: instance triples of SIG 1 ("instance", node name, node value)
        attribute1: attribute triples of SIG 1 (attribute name, node name, attribute value)
        relation1: relation triples of SIG 1 (relation name, node 1 name, node 2 name)
        instance2: instance triples of SIG 2 ("instance", node name, node value)
        attribute2: attribute triples of SIG 2 (attribute name, node name, attribute value)
        relation2: relation triples of SIG 2 (relation name, node 1 name, node 2 name)
        prefix1: prefix label for SIG 1
        prefix2: prefix label for SIG 2
    Returns:
    ...
    # the ith entry is the node index in SIG 2 which maps to the ith node in SIG 1
...
def compute_pool(instance1, attribute1, relation1,
    ...
    mapping one node in SIG 1 to another node in SIG2)

    Arguments:
        instance1: instance triples of SIG 1
        attribute1: attribute triples of SIG 1 (attribute name, node name, attribute value)
        relation1: relation triples of SIG 1 (relation name, node 1 name, node 2 name)
        instance2: instance triples of SIG 2
        attribute2: attribute triples of SIG 2 (attribute name, node name, attribute value)
        relation2: relation triples of SIG 2 (relation name, node 1 name, node 2 name
        prefix1: prefix label for SIG 1
        prefix2: prefix label for SIG 2
    Returns:
      candidate_mapping: a list of candidate nodes.
                       The ith element contains the node indices (in SIG 2) the ith node (in SIG 1) can map to.
    ...
    for i in range(0, len(relation1)):
        for j in range(0, len(relation2)):
            ...
            if relation1[i][0].lower() == relation2[j][0].lower():
                node1_index_sig1 = int(relation1[i][1][len(prefix1):])
                node1_index_sig2 = int(relation2[j][1][len(prefix2):])
                node2_index_sig1 = int(relation1[i][2][len(prefix1):])
                node2_index_sig2 = int(relation2[j][2][len(prefix2):])
                # add mapping between two nodes
                candidate_mapping[node1_index_sig1].add(node1_index_sig2)
                candidate_mapping[node2_index_sig1].add(node2_index_sig2)
                node_pair1 = (node1_index_sig1, node1_index_sig2)
                node_pair2 = (node2_index_sig1, node2_index_sig2)
                if node_pair2 != node_pair1:
                    ...
                    if node1_index_sig1 > node2_index_sig1:
                        # swap node_pair1 and node_pair2
                        node_pair1 = (node2_index_sig1, node2_index_sig2)
                        node_pair2 = (node1_index_sig1, node1_index_sig2)
...
def smart_init_mapping(candidate_mapping, instance1, instance2):
    ...
    Arguments:
        ...
        instance2: instance triples of SIG 2
    Returns:
        initialized node mapping between two SIGs
    ...
    for i, candidates in enumerate(candidate_mapping):
        if len(candidates) == 0:
            ...
            result.append(-1)
            continue
        # node value in instance triples of SIG 1
    ...
    for i in no_word_match:
        candidates = [m for m in candidate_mapping[i] if m not in matched_dict]
        if len(candidates) > 0:
            result[i] = random.choice(candidates)
            matched_dict.add(result[i])
    return result
...
def random_init_mapping(candidate_mapping):
    ...
    Returns:
        randomly-generated node mapping between two SIGs
    ...
    for c in candidate_mapping:
        candidates = [m for m in c if m not in matched_dict]
        if len(candidates) == 0:
...
def compute_match(mapping, weight_dict):
    ...
    Args:
    mappings: a list of node index in SIG 2. The ith element (value j) means node i in SIG 1 maps to node j in SIG 2.
    Returns:
    matching triple number
    Complexity: O(m*n) , m is the node number of SIG 1, n is the node number of SIG 2
    ...
    match_num = 0
    # i is node index in SIG 1, m is node index in SIG 2
    for i, m in enumerate(mapping):
        if m == -1:
            # no node maps to this node
            continue
        # node i in SIG 1 maps to node m in SIG 2
...
def move_gain(mapping, node_id, old_id, new_id, weight_dict, match_num):
    ...
    Arguments:
        ...
        node_id: remapped node in SIG 1
        old_id: original node id in SIG 2 to which node_id is mapped
    ...
    mapping[node_id] = new_id
    match_triple_dict[tuple(mapping)] = match_num + gain
    mapping[node_id] = saved_id
    return gain
...
def swap_gain(mapping, node_id1, mapping_id1, node_id2, mapping_id2, weight_dict, match_num):
    ...
    node_id1: node 1 index in SIG 1
    mapping_id1: the node index in SIG 2 node 1 maps to (in the current mapping)
    node_id2: node 2 index in SIG 1
    mapping_id2: the node index in SIG 2 node 2 maps to (in the current mapping)
    weight_dict: weight dictionary
    ...
    """
    saved_id1 = mapping[node_id1]
    saved_id2 = mapping[node_id2]
...
def get_best_gain(mapping, candidate_mappings, weight_dict, instance_len, cur_match_num):
    ...
    instance_len: the number of the nodes in SIG 2
    ...
    for i, nid in enumerate(mapping):
        # current node i in SIG 1 maps to node nid in SIG 2
        for nm in unmatched:
...
def print_alignment(mapping, instance1, instance2):
    ...
    Args:
        ...
        instance1: nodes of SIG 1
        instance2: nodes of SIG 2
...
def print_errors(mapping, sig1, sig2, prefix1, prefix2):
    (instance1, attribute1, relation1) = sig1
    (instance2, attribute2, relation2) = sig2
    ...
    for i in range(0, len(relation1)):
        for j in range(0, len(relation2)):
            ...
            if relation1[i][0].lower() == relation2[j][0].lower():
                ...
                if mapping[node1_index_sig1] == node1_index_sig2 and mapping[node2_index_sig1] == node2_index_sig2:
                    rel1_match[i] = True
                    rel2_match[j] = True
...
def compute_f(match_num, test_num, gold_num):
    ...
    Compute the f-score based on the matching triple number,
                                 triple number of SIG set 1,
                                 triple number of SIG set 2
    Args:
        match_num: matching triple number
        test_num:  triple number of SIG 1 (test file)
        gold_num:  triple number of SIG 2 (gold file)
    Returns:
...
def main(arguments):
    ...
    # sentence number
    sent_num = 1
    # Read sig pairs from two files
    while True:
        cur_sig1 = get_sig_line(args.f[0])
        cur_sig2 = get_sig_line(args.f[1])
        if cur_sig1 == "" and cur_sig2 == "":
            break
        if cur_sig1 == "":
            print >> ERROR_LOG, "Error: File 1 has less SIGs than file 2"
            print >> ERROR_LOG, "Ignoring remaining SIGs"
            break
        if cur_sig2 == "":
            print >> ERROR_LOG, "Error: File 2 has less SIGs than file 1"
            print >> ERROR_LOG, "Ignoring remaining SIGs"
            break
        sig1 = sig.SIG.parse_SIG_line(cur_sig1)
        sig2 = sig.SIG.parse_SIG_line(cur_sig2)
        prefix1 = "a"
        prefix2 = "b"
        # Rename node to "a1", "a2", .etc
        sig1.rename_node(prefix1)
        # Renaming node to "b1", "b2", .etc
        sig2.rename_node(prefix2)
        (instance1, attributes1, relation1) = sig1.get_triples()
        (instance2, attributes2, relation2) = sig2.get_triples()
        if verbose:
            # print parse results of two SIGs
            print >> DEBUG_LOG, "SIG pair", sent_num
            print >> DEBUG_LOG, "============================================"
            print >> DEBUG_LOG, "SIG 1 (one-line):", cur_sig1
            print >> DEBUG_LOG, "SIG 2 (one-line):", cur_sig2
            print >> DEBUG_LOG, "Instance triples of SIG 1:", len(instance1)
            print >> DEBUG_LOG, instance1
            print >> DEBUG_LOG, "Attribute triples of SIG 1:", len(attributes1)
            print >> DEBUG_LOG, attributes1
            print >> DEBUG_LOG, "Relation triples of SIG 1:", len(relation1)
            print >> DEBUG_LOG, relation1
            print >> DEBUG_LOG, "Instance triples of SIG 2:", len(instance2)
            print >> DEBUG_LOG, instance2
            print >> DEBUG_LOG, "Attribute triples of SIG 2:", len(attributes2)
            print >> DEBUG_LOG, attributes2
            print >> DEBUG_LOG, "Relation triples of SIG 2:", len(relation2)
        ...
        if not single_score:
            # if each SIG pair should have a score, compute and output it here
        ...
        # clear the matching triple dictionary for the next SIG pair
        clear_match_triple_dict()
        sent_num += 1
    if verbose:
        print >> DEBUG_LOG, "Total match number, total triple number in SIG 1, and total triple number in SIG 2:"
        print >> DEBUG_LOG, total_match_num, total_test_num, total_gold_num
        print >> DEBUG_LOG, "---------------------------------------------------------------------------------"
    # output document-level smatch score (a single f-score for all SIG pairs in two files)
    if single_score:
    ...
    if sys.version_info[1] < 7:
        ...
        if args.f is None:
            print >> ERROR_LOG, "smatch.py requires -f option to indicate two files \
                                 containing SIG as input. Please run smatch.py -h to  \
